...
* Rework time handling                                                                   :CLEANUP:
- class MusicalTime
  - rational<int64_t, int64_t>
- BlockContext in VM has time_map
  - MusicalTime time_map[block_size]
  - filled with musical time at each sample position
    - or lower resolution and interpolate with constant BPM
  - filled by player, taking loops, etc. into account
  - time_map[n+1] = time_map[n] + bpm(time_map[n]) / 4 / 60 / sample_rate
