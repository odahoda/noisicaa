...
- csound processors should always clear output buffers
  - can't rely on csound doing it itself
- reimplement set_backend_parameters
- reimplement playing test sound from settings dialog
- port parameters
...
* Status::Error should carry filename:lineno info					 :CLEANUP:
