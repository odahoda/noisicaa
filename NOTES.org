...
* redesign sheet rendering								 :CLEANUP:
...
- project process loads sample data into memory
- rendering
  - render by project process (which has the sample data in memory)
  - don't use a normal command, make it an special call
- SampleEntitySource feeds samples to autoproc
