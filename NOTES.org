...
- how to handling moving a control point past its neighbour?
  - remove it, push it away, restrict movement between neighbours
- splines?
- send commands on changes
- map timepos to x axis
- create a source node in pipeline
- add control data in player
- properties
  - min/max
  - linear/log
  - unit (Hz, dB, %, ..)
...
* TODO Control ports in audio pipeline							      :FR:
- doubles at sample rate
- add new port type
- control source node
- LFO node with just a control output port
- some other node accepting a control input port
...
* Playback pos not displayed on ScoreTrack anymore					     :BUG:
