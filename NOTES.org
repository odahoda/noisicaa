...
- Status::FromErrno()
- make noisicore a single module that can be pyimported
  - just a single module linking in libnoisicore.a
  - don't link test to libnoisicore.a
- implement IPC backend
  - c++ implementation of stream protocol
    - make python wrapper around classes, use those in player
    - change API to make it more proto friendly with less copying
- lv2 features
  - HostData lv2 struct with feature handlers
- event support for lv2 nodes
- parameters:
  - collect parameter changes in list as they come in from main thread
  - at beginning of block, take current list, apply parameters by writing values to buffers
  - what about string parameters?
  - only for kRateControl values?
