* Changing instrument is broken						:BUG:
  File "/home/pink/noisicaa/noisicaa/music/score_track.py", line 54, in run
    track.remove_instrument_from_pipeline()
AttributeError: 'BeatTrack' object has no attribute 'remove_instrument_from_pipeline'

* TODO Track type: percussion						 :FR:
** move beats to arbitrary positions
** move existing beat marker around
** per beat velocity
** set instrument and pitch in TrackProperties dock
* "Hardlink" model for measures within tracks.				 :FR:
...
** is_first, is_last properties from reference object
