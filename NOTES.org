* TODO Track type: percussion						 :FR:
** use "hardlink" model
- track object has unordered list of MeasureContent objects
- list of measures pointing at MeasureContent objects
- reference count MeasureContent objects, garbage collect when zero
** single instrument, fixed pitch
** only emit noteon events
** beats snap to tick markers, but can also be moved to arbitrary times
** view installs listeners on referenced MeasureContent object
** per measure TimeSignature
** TimeSignature determines tick markers
** render as line with beat markers (dots, crosses..?)
** click to add beat marker, shift click to remove
...
* project_fuzztest.py						    :TESTING:
   ...
   * random actions
      ...
      * coverage report
* pylint doesn't work with vext?				    :TESTING:
