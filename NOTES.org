...
* Redesign tool handling                                                                 :CLEANUP:
- Primary tool types:
  - select
    - call it "arrange"?
    - select measures
    - select tracks?
      - or rather have that in the tracks dock?
    - cut, copy, paste, link
    - drag'n'drop move, copy, link
    - fill time range with repeating pattern
    - select elements (notes, beats, control points, ...)? within time range?
      - or rather have that under editing
      - it's track specific
  - edit
    - track specific editing
- Remember selected tool in session
- cut, copy, paste                                                                           :FR:
  - Use QClipboard
...
- Linked measures                                                                             :FR:
  ...
  measure.
* Reopen portaudio stream on block size changes                                              :BUG:
and pass in the block_size
Always opening it with smallest block size didn't work after all, causing constant underruns.
