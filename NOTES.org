...
- cleanup unused code
  - noisicaa.bindings.fluidsynth?
  - noisicaa.audioproc.audio_format?
  - noisicaa.audioproc.[bindings]
- have list of common URIDs in HostData, don't use map() calls in code
- get rid of AST
...
* Debug crashes in gdb									      :FR:
- where are cores dumped? anything to do to enable full core dumps
- if runtests.py crashes, call gdb with core
