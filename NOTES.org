...
- report node deactivation back to player, persist in model
  - model modifications must be a command
    -> would be weird when this can undone...
    - rather in session state?
  -> that gets propagated to UI
  - visualize deactivated nodes in UI
...
* Reanimate piano in instrument library							      :FR:
So I can preview instruments again
