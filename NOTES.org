...
of actual measures.
- views need listener on ref's measure property, to update when it changes.
- reinsert existing measure.
- basic copy'n'paste
  - only 'copy' & 'paste as linked'
  - possibly a range of measures within a single track
  - copied items are property of project
    - MeasureItem.getCopy
      - fetch serialized copy of objects from backend
      - remember relative positions
