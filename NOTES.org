...
* stats module										      :FR:
- each process has StatsTracker, that only contains stats
- ProcessManager maintains ChildConnection based on a pair of anonymous pipes
  - use it for the start barrier, response is server address
  - ProcessImpl spins up thread to listen to further commands from ChildConnection
    - 'COLLECT_STATS'
- ProcessManager collects stats from each process
  - StatsCollector class
  - stats thread that polls all childs with a child connection
