...
class ProcessorMixerTest(
        unittest_processor_mixins.ProcessorTestMixin,
        unittest.TestCase):
    def test_process_block(self):
        self.node_description = self.node_db['builtin://mixer']
        self.create_processor()
        self.fill_buffer('in:left', 1.0)
        self.fill_buffer('in:right', -1.0)
        self.fill_buffer('gain', 0.0)
        self.fill_buffer('pan', 0.0)
        self.fill_buffer('lp_cutoff', 15000.0)
        self.fill_buffer('hp_cutoff', 50.0)
        self.process_block()
