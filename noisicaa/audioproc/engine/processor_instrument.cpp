...
ProcessorInstrument::ProcessorInstrument(
  ...
  : Processor(
      node_id, "noisicaa.audioproc.engine.processor.instrument", host_system, desc),
...
Status ProcessorInstrument::handle_message_internal(pb::ProcessorMessage* msg) {
  switch (msg->msg_case()) {
  case pb::ProcessorMessage::kChangeInstrument:
    return change_instrument(msg->change_instrument().instrument_spec());
    return Status::Ok();
...
endin
    ...
    instrument->csound.reset(new CSoundUtil(_host_system));
