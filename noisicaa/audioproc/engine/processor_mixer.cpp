...
ProcessorMixer::ProcessorMixer(
  ...
  : ProcessorCSoundBase(
      node_id, "noisicaa.audioproc.engine.processor.mixer", host_system, desc),
    _muted(false) {}
...
Status ProcessorMixer::handle_message_internal(pb::ProcessorMessage* msg) {
  switch (msg->msg_case()) {
  case pb::ProcessorMessage::kMuteNode:
...
Status ProcessorMixer::process_block_internal(BlockContext* ctxt, TimeMapper* time_mapper) {
  if (!_muted.load()) {
    RETURN_IF_ERROR(ProcessorCSoundBase::process_block_internal(ctxt, time_mapper));
  } else {
    clear_all_outputs();
  }
  ...
  lv2_atom_forge_push(&forge, &frame, lv2_atom_forge_atom(&forge, 0, _meter_urid));
  for (int ch = 0 ; ch < 2 ; ++ch) {
