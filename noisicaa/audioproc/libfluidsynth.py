...
# TODO: needs cleanup (or rewrite in cython?)
# pylint: skip-file
...
fluid_synth_remove_sfont = cfunc('fluid_synth_remove_sfont', None,
                                 ('synth', c_void_p, 1),
                                 ('sfont', c_void_p, 1))
...
# Install our own log functions and send message to the logging system.
...
def log_func(level, msg, data):
    msg = msg.decode('ascii')
    if level == 2 and msg.startswith("No preset found on channel"):
        # Ignore this super spammy and useless warning.
        return
    logger.log(_level_map[level], "%s", msg)

fluid_set_log_function = cfunc('fluid_set_log_function', c_void_p,
                               ('level', c_int, 1),
                               ('fun', c_void_p, 1),
                               ('data', c_void_p, 1))

for level in range(5):
    fluid_set_log_function(level, log_func, None)
...
class Synth:
    ...
    def start(self, driver=None):
        ...
        if driver is not None:
            ...
            fluid_settings_setstr(self.settings, b'audio.driver', driver.encode('ascii'))
    ...
    def sfload(self, filename, update_midi_preset=0):
        ...
        return fluid_synth_sfload(self.synth, filename.encode('ascii'), update_midi_preset)
    ...
    def get_sfont(self, sfid):
        return fluid_synth_get_sfont_by_id(self.synth, sfid)
    def add_sfont(self, sfont):
        return fluid_synth_add_sfont(self.synth, sfont)
    def remove_sfont(self, sfont):
        fluid_synth_remove_sfont(self.synth, sfont)
    ...
    def noteon(self, chan, key, vel):
        ...
        if chan < 0:
            raise ValueError("Bad channel %r" % channel)
        if not (0 <= vel <= 127):
            raise ValueError("Bad velocity %r" % vel)
