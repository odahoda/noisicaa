...
AudioFileSubSystem::AudioFileSubSystem()
  : _logger(LoggerRegistry::get_logger("noisicaa.audioproc.vm.audio_file")) {}

AudioFileSubSystem::~AudioFileSubSystem() {
  cleanup();
}

Status AudioFileSubSystem::setup() {
  return Status::Ok();
}
...
StatusOr<AudioFile*> AudioFileSubSystem::load_audio_file(const string& path) {
  ...
  uint32_t num_samples = av_rescale_rnd(sfinfo.frames, 44100, sfinfo.samplerate, AV_ROUND_UP);
  ...
  SwrContext* ctxt = swr_alloc_set_opts(
      nullptr,
      av_get_default_channel_layout(sfinfo.channels), AV_SAMPLE_FMT_FLTP, 44100,
