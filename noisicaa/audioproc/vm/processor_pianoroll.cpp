...
void PianoRoll::apply_mutation(pb::ProcessorMessage* msg) {
  ...
  // Invalidate pianoroll's cursor (so ProcessorPianoRoll::run() is forced to do a seek first).
  offset = -1;
}
...
ProcessorPianoRoll::ProcessorPianoRoll(const string& node_id, HostData* host_data)
  : Processor(node_id, "noisicaa.audioproc.vm.processor.pianoroll", host_data),
    _pianoroll_manager(_logger) {}

ProcessorPianoRoll::~ProcessorPianoRoll() {}

Status ProcessorPianoRoll::setup(const ProcessorSpec* spec) {
...
void ProcessorPianoRoll::cleanup() {
  Processor::cleanup();
}
...
Status ProcessorPianoRoll::connect_port(uint32_t port_idx, BufferPtr buf) {
  assert(port_idx == 0);
  _out_buffer = buf;
  return Status::Ok();
}
...
void ProcessorPianoRoll::note_on(
  ...
  lv2_atom_forge_atom(forge, 3, _host_data->lv2->urid.midi_event);
...
Status ProcessorPianoRoll::run(BlockContext* ctxt, TimeMapper* time_mapper) {
  ...
  lv2_atom_forge_init(&forge, &_host_data->lv2->urid_map);

  LV2_Atom_Forge_Frame frame;
  lv2_atom_forge_set_buffer(&forge, _out_buffer, 10240);

  lv2_atom_forge_sequence_head(&forge, &frame, _host_data->lv2->urid.atom_frame_time);
