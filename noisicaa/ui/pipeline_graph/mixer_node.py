...
class MixerNodeWidget(ui_base.ProjectMixin, QtWidgets.QWidget):
    def __init__(self, node: music.BasePipelineGraphNode, **kwargs: Any) -> None:
        ...
        self.__node_msg_listener = self.audioproc_client.node_messages.add(
            '%08x' % self.__node.id, self.__nodeMessage)
    ...
    def __nodeMessage(self, msg: bytes) -> None:
        msg_atom = lv2.wrap_atom(self.app.urid_mapper, msg)
        if msg_atom.type_urid == self.__meter_urid:
            current_left_atom, peak_left_atom, current_right_atom, peak_right_atom = msg_atom.tuple
            assert current_left_atom.type_urid == self.__float_urid
            current_left = current_left_atom.float
            assert peak_left_atom.type_urid == self.__float_urid
            peak_left = peak_left_atom.float
            assert current_right_atom.type_urid == self.__float_urid
            current_right = current_right_atom.float
            assert peak_right_atom.type_urid == self.__float_urid
            peak_right = peak_right_atom.float
...
class MixerNode(base_node.Node):
    def __init__(self, *, node: music.BasePipelineGraphNode, **kwargs: Any) -> None:
        self.__session_prefix = 'node/%08x/' % node.id
