...
cdef class BufferManager(object):
    ...
    def allocate_from_node_description(self, node_description, prefix=''):
        for port in node_description.ports:
            type = {
                node_db.PortDescription.AUDIO: buffers.PyFloatAudioBlock(),
                node_db.PortDescription.ARATE_CONTROL: buffers.PyFloatAudioBlock(),
                node_db.PortDescription.KRATE_CONTROL: buffers.PyFloat(),
                node_db.PortDescription.EVENTS: buffers.PyAtomData(),
            }[port.type]
